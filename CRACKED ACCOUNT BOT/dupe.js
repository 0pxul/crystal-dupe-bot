...
const ITEMS_TO_REFILL = {
    'enchanted_golden_apple': { name: 'enchanted_golden_apple', shulkerPos: new Vec3( -1 ) }, // REPLACE -1 WITH EGAPS SHULKER CORDS
    'end_crystal': { name: 'end_crystal', shulkerPos: new Vec3( -1 ) }, // REPLACE -1 WITH CRYSTAL SHULKER CORDS
    'chest': { name: 'chest', shulkerPos: new Vec3( -1 ) }, // REPLACE -1 WITH CHEST SHULKER CORDS
    'experience_bottle': { name: 'experience_bottle', shulkerPos: new Vec3( -1 ) } // REPLACE -1 WITH XP SHULKER CORDS
};
const ARMOR_SLOTS = [5, 6, 7, 8]; // Helmet, Chestplate, Leggings, Boots
const DURABILITY_THRESHOLD = 0.7; // 70%
const chestPos = new Vec3( -1 ); // REPLACE -1 WITH YOUR CHEST CORDS
const crystalPos = new Vec3( -1 ); // REPLACE -1 WITH THE OBSIDIAN THAT THE CRYSTAL WILL BE ON 
module.exports.dupe = async function (bot) {
    ...
    while (true) {
        ...
        for (const key in ITEMS_TO_REFILL) {
            ...
            if (itemCount <= 10) { // XP REFILL COUNT
